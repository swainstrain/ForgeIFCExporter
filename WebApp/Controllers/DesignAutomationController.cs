...
namespace WebApp.Controllers
{
    ...
    public class DesignAutomationController : ControllerBase
    {
            ...
            if (!activities.Data.Contains(qualifiedActivityId))
            {
                ...
                Activity activitySpec = new Activity()
                {
                    ...
                    Parameters = new Dictionary<string, Parameter>()
                    {
                        ...
                        { "outputFile", new Parameter() { Description = "output file", LocalName = "outputFile.ifc", Ondemand = false, Required = true, Verb = Verb.Put, Zip = false } }
                    },
        ...
        public async Task<IActionResult> StartWorkitem([FromForm] StartWorkitemInput input)
        {
            ...
            string bucketKey = NickName.ToLower() + "-rvt_input";
            ...
            try
            {
                ...
                await buckets.CreateBucketAsync(bucketPayload, "US");
                       ...
                       // 2. upload inputFile
            string inputFileNameOSS = string.Format("{1}", DateTime.Now.ToString("yyyyMMddhhmmss"), Path.GetFileName(input.inputFile.FileName)); // avoid overriding
            ...
            // 3. output file
            string bucketOut = NickName.ToLower() + "-ifc_output";
            try
            {
                PostBucketsPayload bucketPayload = new PostBucketsPayload(bucketOut, null, PostBucketsPayload.PolicyKeyEnum.Transient);
                await buckets.CreateBucketAsync(bucketPayload, "US");
            }
            catch { }; // in case bucket already exists


            string outputFileNameOSS = string.Format("{1}", DateTime.Now.ToString("yyyyMMddhhmmss"), Path.GetFileName(input.inputFile.FileName).Replace(".rvt",".ifc")); // avoid overriding

            XrefTreeArgument outputFileArgument = new XrefTreeArgument()
            {
                Url = string.Format("https://developer.api.autodesk.com/oss/v2/buckets/{0}/objects/{1}", bucketOut, outputFileNameOSS),
                Verb = Verb.Put,
                Headers = new Dictionary<string, string>()
                   {
                       {"Authorization", "Bearer " + oauth.access_token }
        ...
        public async Task<IActionResult> OnCallback(string id, string outputFileName, [FromBody] dynamic body)
        {
            try
            {
                ...
                dynamic signedUrl = await objectsApi.CreateSignedResourceAsyncWithHttpInfo(NickName.ToLower() + "-ifc_output", outputFileName, new PostBucketsSigned(10), "read");
